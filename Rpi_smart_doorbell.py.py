...
# Rekognition Client
rekog_client = boto3.client("rekognition", region_name='us-east-1', aws_access_key_id='####################',
                aws_secret_access_key='########################################')
                
# S3 Configuration
s3_client = boto3.client('s3', region_name='us-east-1', aws_access_key_id='####################',
                aws_secret_access_key='########################################')
s3_bucket = "videostreambucketpankesh"
...
def convert_to_bytearray():
    ...
    """
    frame = cv2.imread('output/images/'+str(time_stamp)+'/1.jpg')
...
def Face_detection(img_bytes):
    ...
    if len(faceDetectionResponse['FaceDetails']) != 0:
        ...
        if rekog_face_response['FaceMatches']:
            print('Hello, ',rekog_face_response['FaceMatches'][0]['Face']['ExternalImageId'])
...
def Upload_to_aws(l):
    ...
    for i in range(len(final_name)):
        try:
            #final_notification_type.remove('')
            final_name.remove('')
        
        except ValueError:
            pass
    
    #notification_type = ', '.join(final_notification_type)
    name = ''
    notification_type = ''
    notification_message = ''
    notification_title = ''
